...
def create_item(
    *,
    ...
    Create new item.
    """
    item = service.item.create_with_owner(db=db, obj_in=item_in, owner_id=current_user.id)
    return item
...
def update_item(
    *,
    ...
    if not service.user.is_superuser(current_user) and (item.owner_id != current_user.id):
